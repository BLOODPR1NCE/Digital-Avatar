...
class ModelTrainer:
    ...
    def prepare_dataloaders(self, samples, batch_size = 8, train_split = 0.8):
        ...
        val_loader = DataLoader(LipSyncDataset(samples[train_size:]), batch_size=batch_size, shuffle=False, collate_fn=collate_fn, drop_last=True)
    ...
    def _train_epoch(self, model, loader, criterion, optimizer):
        ...
        for audio, visual in loader:
            ...
            optimizer.zero_grad()
            loss = criterion(model(audio), visual)
            loss.backward()
